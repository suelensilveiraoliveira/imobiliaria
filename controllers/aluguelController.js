...
export const aluguelCreate = async (req, res) => {
  const { nome, cpf, telefone, imovel_id, preco, endereco } = req.body
  if (!nome || !cpf || !telefone || !imovel_id || !preco || !endereco ) {
      res.status(400).json({ id: 0, msg: "Insira os dados completo" })
  }
  const imovel = await Imovel.findOne({ where: { id: imovel_id } })
  if (imovel.estatus== false) {
      ...
      try {
          ...
          if (!inquilinoExistente) {
            const inquilino = await Inquilino.create({
                  nome, cpf, telefone, imovel_id, endereco
              }, { transaction: t })
              var aluguel = await Aluguel.create({
                  proprietario_id: imovel.proprietario_id, inquilino_id: inquilino.id, imovel_id, preco
              }, { transaction: t })
          } else {
              var aluguel = await Aluguel.create({
                  proprietario_id: imovel.proprietario_id, inquilino_id: inquilinoExistente.id, imovel_id, preco
              }, { transaction: t })
          }
          await Imovel.update({ estatus: true }, { where: { id: imovel_id }, transaction: t })
          await t.commit();
          res.status(200).json(aluguel)
      } catch (error) {
        await t.rollback();
    ...
    res.status(400).send("Imovel já esta alugado")
  }
}
export const aluguelUpdateFinaliza = async (req, res) => {
  ...
  try {
      ...
      if (aluguel) {
          await Aluguel.update({ data_final: new Date() }, { where: { id: id }, transaction: t })
          await Imovel.update({ estatus: false }, { where: { id: aluguel.imovel_id }, transaction: t })
      } else {
          await t.rollback()
          res.status(400).send("Imóvel ainda não cadastrado")
      }
      await t.commit()
      res.status(200).json(aluguel)
  } catch (error) {
